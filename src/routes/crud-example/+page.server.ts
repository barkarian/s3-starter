...
export const load: PageServerLoad = async (event) => {
    // Server API:
    const [createForm, updateForm, removeForm] =
        await Promise.all([
    ...
    let guiPaginationData = await findConfigurations(
        limit,
        page,
        event
    )
...
export const actions: Actions = {
    create: async (event) => {
        const { data, error, form } = await createConfiguration(
            event
        )
        if (
            error?.status == Number(PUBLIC_INVALID_FORM_STATUS)
        ) {
    ...
    update: async (event) => {
        const { data, error, form } = await updateConfiguration(
            event
        )
        if (
            error?.status == Number(PUBLIC_INVALID_FORM_STATUS)
    ...
    delete: async (event) => {
        const { data, error, form } = await deleteConfiguration(
            event
        )
        if (
            error?.status == Number(PUBLIC_INVALID_FORM_STATUS)
